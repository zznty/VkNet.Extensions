    ...
    : IVkApiInvoke
{
    ...
    private async Task<T> CallAsync<T>(string methodName, VkParameters parameters, bool skipAuthorization, IEnumerable<JsonConverter> jsonConverters)
    {
    ...
    private async Task<JToken> InvokeInternalAsync(string methodName, IDictionary<string, string> parameters, bool skipAuthorization)
    {
        ...
        return await handler.Perform(async (sid, key) =>
        {
            ...
            {
                if (await tokenRefreshHandler.RefreshTokenAsync(tokenStore.Token) is not { } newToken)
                    throw;
