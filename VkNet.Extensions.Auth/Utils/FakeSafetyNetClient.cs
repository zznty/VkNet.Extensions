...
using ProtoBuf;
using VkNet.Extensions.Auth.Models.Google;
...
internal class FakeSafetyNetClient : IDisposable
{
	...
	public FakeSafetyNetClient()
	{
		_httpClient = new()
		{
			BaseAddress = new("https://android.clients.google.com/")
		};
	...
	public async Task<AndroidCheckinResponse> CheckIn()
	{
		...
		var response = await _httpClient.PostAsync("checkin", content);
	...
	public async Task<string> Register(AndroidCheckinResponse credentials)
	{
		...
		var httpRequestMessage = new HttpRequestMessage(HttpMethod.Post, "c2dm/register3")
		{
			Content = content
		};
